...
기존 로그인 토큰을 사용하며 앱 등록, 설정 저장, 배포, 승인, 중단을 실제 배포 API로 요청한다.
웹은 롤백 대신 리비전 탭에서 과거 설정을 선택해 새 배포를 실행한다. 최신 저장 설정은 하나만 유지한다.
...
파일은 `apps` 배열과 앱 ID별 `configurations`·`runs` 객체, 선택적인 `revisions`·`executions`·`templates` 객체를 포함한다. 저장 설정은 최신 하나만
...
블루그린 승격, 웹 롤백 제거, 과거 리비전의 새 실행과 최신 설정 불변성, 앱 등록, 설정 저장, 기본/가중치 카나리, 웹/배치 목록 분리, 그룹 배포와
